...
function hero(){
  const title=E(state.config?.hero_title||"Học tiếng Anh. Kết nối cộng đồng. Phát triển cùng nhau.");
  const text=E(state.config?.hero_text||"Cổng thông tin chính thức của Câu lạc bộ Tiếng Anh The Sky First (SFEC), nơi cung cấp thông tin về lớp học, hoạt động, chương trình, cơ hội tham gia và các nội dung dành cho học viên, thành viên và cộng đồng.");
  return `<section class="hero hero-official"><div class="hero-copy"><span class="eyebrow">CỔNG THÔNG TIN CHÍNH THỨC</span><div class="hero-org">CÂU LẠC BỘ TIẾNG ANH THE SKY FIRST (SFEC)</div><h1>${title}</h1><p>${text}</p><p class="hero-owner"><b>SFEC là đơn vị thuộc Mạng lưới Giáo dục & Phát triển Cộng đồng Sky First (SFN)</b>, hướng đến xây dựng môi trường học tập tiếng Anh tích cực, kết nối người trẻ và tạo cơ hội phát triển thông qua giáo dục và hoạt động cộng đồng.</p><p class="hero-slogan"><b>${E(state.config?.slogan||"Sky is not the limit – it’s just the beginning.")}</b></p><div class="actions"><a class="primary" href="#classes">Khám phá lớp học</a><a class="secondary" href="#forms">Tham gia SFEC</a><a class="hero-link" href="#about">Giới thiệu SFEC →</a></div></div></section>`;
}
...
function renderInfoPage(kind){const pages={about:{k:"GIỚI THIỆU SFEC",t:"Một không gian học tiếng Anh và phát triển cùng nhau",b:`<p>Câu lạc bộ Tiếng Anh The Sky First (SFEC) là môi trường học tập, thực hành tiếng Anh và kết nối cộng đồng dành cho học viên, thành viên và những người đồng hành.</p><p>SFEC là đơn vị thuộc <b>Mạng lưới Giáo dục & Phát triển Cộng đồng Sky First (SFN)</b>. Hoạt động của Câu lạc bộ được tổ chức trong hệ sinh thái Sky First, gắn giáo dục với trải nghiệm, trách nhiệm và phát triển cộng đồng.</p>`},journey:{k:"HÀNH TRÌNH",t:"Hành trình phát triển của SFEC",b:`<p>Hành trình SFEC được xây dựng theo từng giai đoạn hoạt động, từ tổ chức lớp học, phát triển cộng đồng học tập đến hoàn thiện hệ thống vận hành và kết nối trong Mạng lưới Sky First.</p><div class="timeline"><div><b>Khởi tạo</b><p>Hình thành môi trường học tập và kết nối tiếng Anh.</p></div><div><b>Phát triển</b><p>Mở rộng lớp học, hoạt động, đội ngũ và cơ hội tham gia.</p></div><div><b>Kết nối hệ thống</b><p>Vận hành như một đơn vị thuộc Mạng lưới Giáo dục & Phát triển Cộng đồng Sky First (SFN).</p></div></div>`},values:{k:"ĐỊNH HƯỚNG & GIÁ TRỊ",t:"Học tập tích cực, kết nối có trách nhiệm",b:`<div class="grid3"><div class="card"><h3>Học tập</h3><p class="muted">Tạo môi trường thực hành và phát triển năng lực tiếng Anh.</p></div><div class="card"><h3>Kết nối</h3><p class="muted">Khuyến khích hợp tác, chia sẻ và đồng hành trong cộng đồng.</p></div><div class="card"><h3>Phát triển</h3><p class="muted">Tạo cơ hội học hỏi từ trải nghiệm và hoạt động cộng đồng.</p></div></div>`},organization:{k:"CƠ CẤU & ĐƠN VỊ CHỦ QUẢN",t:"SFEC trong hệ sinh thái Sky First",b:`<div class="card org-card"><span class="pill">ĐƠN VỊ CHỦ QUẢN</span><h3>Mạng lưới Giáo dục & Phát triển Cộng đồng Sky First (SFN)</h3><p class="muted">SFEC là đơn vị thuộc SFN và kết nối với các hệ thống chung của Mạng lưới.</p><div class="actions"><a class="primary" href="https://www.skyfirst.io.vn" target="_blank" rel="noopener">Sky First Network</a><a class="secondary" href="https://tnv.skyfirst.io.vn" target="_blank" rel="noopener">Cổng Tình nguyện viên</a></div></div>`}};const x=pages[kind]||pages.about;app.innerHTML=`<div class="info-page"><div class="section-kicker">${x.k}</div><h1>${x.t}</h1><div class="info-copy">${x.b}</div></div>`}
...
function renderLookup(){
  app.innerHTML=`<div class="lookup-page"><div class="section-kicker">TRA CỨU & XÁC MINH</div><h1>Tra cứu thông tin</h1><p class="muted">Tra cứu trạng thái hồ sơ đăng ký hoặc xác minh Giấy chứng nhận được ghi nhận trong hệ thống SFEC.</p><div class="grid2 lookup-grid">
  <div class="card lookup-card"><span class="lookup-label">HỒ SƠ ĐĂNG KÝ</span><h2>Tra cứu hồ sơ</h2><p class="muted">Sử dụng mã hồ sơ và email đã dùng khi đăng ký.</p><form id="lookupRecord"><div class="field"><label>Mã hồ sơ</label><input name="code" required placeholder="SFEC-...-2026-0001"></div><div class="field"><label>Email đã đăng ký</label><input name="email" type="email" required></div><button class="primary">Tra cứu hồ sơ</button></form><div id="recordResult"></div></div>
  <div class="card lookup-card"><span class="lookup-label">GIẤY CHỨNG NHẬN</span><h2>Xác minh Giấy chứng nhận</h2><p class="muted">Nhập chính xác mã Giấy chứng nhận để kiểm tra thông tin xác minh công khai.</p><form id="lookupCert"><div class="field"><label>Mã Giấy chứng nhận</label><input name="code" required placeholder="001/GCN-SFEC/2026"></div><button class="primary">Xác minh</button></form><div id="certResult"></div></div></div></div>`;
  document.getElementById("lookupRecord").onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);try{const d=await api(`/api/lookup/submission?code=${encodeURIComponent(f.get("code"))}&email=${encodeURIComponent(f.get("email"))}`);document.getElementById("recordResult").innerHTML=`<div class="lookup-result valid"><div class="verify-head"><span class="verify-dot"></span><div><small>TRẠNG THÁI HỒ SƠ</small><strong>${E(d.item.status)}</strong></div></div><dl><div><dt>Mã hồ sơ</dt><dd>${E(d.item.code)}</dd></div><div><dt>Loại hồ sơ</dt><dd>${E(d.item.form_id)}</dd></div><div><dt>Ngày tiếp nhận</dt><dd>${fmt(d.item.created_at)}</dd></div><div><dt>Cập nhật gần nhất</dt><dd>${fmt(d.item.updated_at)}</dd></div></dl></div>`}catch{document.getElementById("recordResult").innerHTML="<div class='notice bad'>Không tìm thấy hồ sơ phù hợp với mã và email đã nhập.</div>"}};
  document.getElementById("lookupCert").onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);try{const d=await api(`/api/lookup/certificate?code=${encodeURIComponent(f.get("code"))}`);const c=d.item,m=J(c.metadata_json)||{};const valid=c.status!=="revoked";document.getElementById("certResult").innerHTML=`<div class="lookup-result ${valid?'valid':'revoked'}"><div class="verify-head"><span class="verify-dot"></span><div><small>TRẠNG THÁI XÁC MINH</small><strong>${valid?'Giấy chứng nhận hợp lệ':'Giấy chứng nhận đã thu hồi'}</strong></div></div><dl><div><dt>Mã Giấy chứng nhận</dt><dd>${E(c.code)}</dd></div><div><dt>Họ và tên</dt><dd>${E(c.full_name)}</dd></div><div><dt>Nội dung ghi nhận</dt><dd>${E(c.content)}</dd></div><div><dt>Đơn vị</dt><dd>${E(m.unit_name||'Câu lạc bộ Tiếng Anh The Sky First (SFEC)')}</dd></div><div><dt>Đơn vị chủ quản</dt><dd>${E(m.owner_name||'Mạng lưới Giáo dục & Phát triển Cộng đồng Sky First (SFN)')}</dd></div><div><dt>Ngày cấp</dt><dd>${fmt(c.issued_at)}</dd></div>${m.registry_no?`<div><dt>Số vào sổ</dt><dd>${E(m.registry_no)}</dd></div>`:''}${m.signer?`<div><dt>Người ký</dt><dd>${E(m.signer)}</dd></div>`:''}${m.signer_title?`<div><dt>Chức danh</dt><dd>${E(m.signer_title)}</dd></div>`:''}<div><dt>Trạng thái</dt><dd>${E(c.status)}</dd></div></dl></div>`}catch{document.getElementById("certResult").innerHTML="<div class='notice bad'>Không tìm thấy mã Giấy chứng nhận.</div>"}};
...
const adminMenu=[
 ...
 {group:"NỘI DUNG",items:[["news","Bản tin & CMS"],["forms","Form Builder"],["documents","Kho văn bản"],["files","File & Minh chứng"]]},
